...
if (isset($_GET['table']) && $_GET['table'] == 'monthly_leaderboard') {
    ...
    $sql = "SELECT u.email,u.name,u.profile,r.* FROM (
        ...
        ( SELECT m.id, user_id, sum(score) score,last_updated,date_created FROM monthly_leaderboard m join users u on u.id = m.user_id $where_sub GROUP BY user_id ORDER BY score DESC) s,
...
if (isset($_GET['table']) && $_GET['table'] == 'contest') {
    ...
    foreach ($res as $row) {
        ...
        $tempRow['contest_type'] = $row['contest_type'];
...
if (isset($_GET['table']) && $_GET['table'] == 'maths_junior_question') {
    ...
    $sql = "SELECT COUNT(q.id) as total FROM `tbl_maths_junior_question` q " . $left_join . " " . $where;
    $db->sql($sql);
    $res = $db->getResult();
    foreach ($res as $row) {
        $total = $row['total'];
    ...
    $sql = "SELECT q.*, l.language FROM `tbl_maths_junior_question` q " . $left_join . " " . $where . " ORDER BY " . $sort . " " . $order . " LIMIT " . $offset . ", " . $limit;
